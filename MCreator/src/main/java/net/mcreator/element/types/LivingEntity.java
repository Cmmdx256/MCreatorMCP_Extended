...
@SuppressWarnings({ "unused", "NotNullFieldNotInitialized" }) public class LivingEntity extends GeneratableElement
	...
	public LivingEntity(ModElement element) {
		...
		this.mobModelName = "Biped";
		this.mobModelTexture = "";
		this.mobName = element != null ? element.getName() : "";
		this.mobLabel = this.mobName;
		this.mobSpawningType = "creature";
		this.mobCreatureType = "UNDEFINED";
		this.mobBehaviourType = "Mob";
		this.aixml = "<xml xmlns=\"https://developers.google.com/blockly/xml\"><block type=\"aitasks_container\" deletable=\"false\" x=\"40\" y=\"40\"></block></xml>";
		...
		this.mobDrop = new net.mcreator.element.parts.MItemBlock();
		this.equipmentMainHand = new net.mcreator.element.parts.MItemBlock();
		this.equipmentOffHand = new net.mcreator.element.parts.MItemBlock();
		this.equipmentHelmet = new net.mcreator.element.parts.MItemBlock();
		this.equipmentBody = new net.mcreator.element.parts.MItemBlock();
		this.equipmentLeggings = new net.mcreator.element.parts.MItemBlock();
		this.equipmentBoots = new net.mcreator.element.parts.MItemBlock();
		this.rangedAttackItem = new net.mcreator.element.parts.MItemBlock();
		this.aiBase = "(none)";
		this.bossBarColor = "PINK";
		this.bossBarType = "PROGRESS";
	}

	@Override @Nullable public Model getEntityModel() {
		if (mobModelName == null || mobModelName.trim().isEmpty())
			mobModelName = "Biped";
		...
		Model model = Model.getModelByParams(getModElement().getWorkspace(), mobModelName, modelType);
		if (model == null && modelType != Model.Type.BUILTIN) {
			model = Model.getModelByParams(getModElement().getWorkspace(), "Biped", Model.Type.BUILTIN);
		}
		return model;
	}
